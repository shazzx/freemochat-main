    ...
    return (
        <div className={`min-w-[260px] sm:max-w-[460px] w-full flex h-full ${chatOpen && "hidden lg:flex"}`}>
            ...
            <div className='relative h-full w-full border border-muted '>
                                ...
                                {chatList?.groups?.length > 0 ? chatList?.groups?.map((chat) => (
                                    <div className="flex gap-2 cursor-pointer w-full p-4 bg-card hover:bg-accent" key={chat?._id} onClick={() => {
                                        ...
                                        if(chat?.recepient?._id){
                                            setRecepientDetails({ userId: chat?.recepient?._id, groupId: chat?.recepient?._id, images: {profile: chat?.recepient?.profile, cover: chat?.recepient?.cover}, name: chat?.recepient?.name, description: chat?.recepient?.description, type: "ChatGroup", removed: chat.removedAt ? true : false })
                                            setChatOpen(true)
                                        }
                                    }}>
                                        <div>
                ...
                {/* {
                    friendListState && friends?.length > 0 &&
                    <div className="absolute top-28 left-0 w-full bg-card">
                        {
                            friendListState && friends?.length > 0 ?
                                friends.map((friend) => (

                                    <div className="flex items-center cursor-pointer gap-4 w-full px-4 py-2 hover:bg-muted" onClick={() => {
                                        setRecepientDetails({ userId: friend?._id, username: friend.username, profile: friend?.images?.profile, fullname: friend.firstname + " " + friend.lastname })
                                        setChatOpen(true)
