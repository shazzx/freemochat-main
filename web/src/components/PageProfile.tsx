...
import { useFollowPage, usePage } from '@/hooks/Post/usePage'
import { usePageFollowers } from '../hooks/Post/usePage'
...
function PageProfile() {
    ...
    const tabList = [{ value: "posts", name: "Posts" }, { value: "followers", name: "Followers" }, { value: "media", name: "Media" }, { value: "about", name: "About" }]

    console.log(setFollowersCount, friendsData, followersData)
    ...
    const _pageData = usePage(handle)
    const { data, isLoading } = usePagePosts("page", _pageData.data?._id)
    const { mutate } = useFollowPage()
    const pageFollowers = usePageFollowers(_pageData.data?._id)
    ...
    const _createPost = async ({ content, selectedMedia, formData }) => {
        let postDetails = { content, type: "page", targetId: _pageData.data._id }
    ...
    const recepientDetails = { pageId: _pageData?.data?._id, hanlde: _pageData?.data?.handle, type: "", fullname: "", name: _pageData?.data?.name }
