                        ...
                        <Card x-chunk="dashboard-01-chunk-5" className="border-none">
                            <CardContent className="grid gap-8 w-full bg-background p-0">
                                ...
                                {friends.length > 0 ? friends?.map((page) => {
                                    ...
                                    <div className="flex items-center gap-4 cursor-pointer w-full px-4 py-2 hover:bg-muted" onClick={() => {
                                        console.log(friend)
                                        setRecepientDetails({ userId: friend?._id, username: friend?.username, profile: friend?.images?.profile, fullname: friend.firstname + " " + friend.lastname, firstname: friend.firstname, lastname: friend.lastname, type: "User" })
                                        setChatOpen(true)
                                    }}>
                                        <Avatar className="hidden h-9 w-9 sm:flex">
                                            <AvatarImage src={friend?.images?.profile} alt="Avatar" />
                                            <AvatarFallback>{friend?.firstname[0]?.toUpperCase() + friend?.lastname[0]?.toUpperCase()}</AvatarFallback>
                                        </Avatar>
                                    ...
                                    : }*/
                                        :
                                        <div className="w-full h-full bg-background flex flex-col items-center pt-10 justif-center">
...
<span>No Friends</span>
                    ...
                    <TabsContent value="groups" className="h-full">
                        {groups?.length > 0 ? groups?.map((group) => (
                            <div className="flex items-center gap-4 cursor-pointer w-full px-4 py-2 hover:bg-muted" onClick={() => {
                                setRecepientDetails({ userId: group?.user, groupId: group?._id, images: group?.images, type: "ChatGroup", name: group?.name })
                                setChatOpen(true)
                                ...
                                <Avatar className="hidden h-9 w-9 sm:flex">
                                    ...
                                    <AvatarFallback>{group?.name[0]?.toUpperCase() + group?.name[1]?.toUpperCase()}</AvatarFallback>
                                </Avatar>
                                <div className="grid gap-1">
                                    <p className="text-sm font-medium leading-none ">
                                        {group.name}
