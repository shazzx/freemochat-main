...
function FriendRequests({ setFriendRequestState }) {
    ...
    return (
        <div className="absolute top-0 right-0 h-screen w-screen z-50">
            ...
            <Card className=" p-1 w-full h-full md:w-[360px] bg-background md:h-[400px] absolute top-14 md:top-9 z-50 md:right-36 sm:border-2 sm:border-accent">
                <CardHeader className="p-4">
                    ...
                    {/* <CardDescription>{isSuccess && data?.length > 1 ? ((data?.length - 1) * 12) + data[data.length - 1].data.length : data[0].data.length}</CardDescription> */}
                </CardHeader>
                <CardContent className="grid gap-4 p-1 ">
                    <div>
                        {!isLoading && isSuccess && data.map((page, i) => {
                            return page?.friendRequests?.map((request, re) => {
                                ...
                                return (
                                    <div className="p-2 flex justify-between w-full bg-card text-white">
                                        ...
                                        <div className="flex items-center justify-center">
                                            ...
                                            }}>{request?.accepted ? "Accepted" : "Accept"}</Button>
                                        </div>
                                    </div>
                                )
                            })
                        ...
                        {!isLoading && isSuccess && data[0].friendRequests.length == 0 &&
