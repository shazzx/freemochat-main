...
function Newfeed() {
    ...
    const { firstname, lastname, username, email, images, bio, followers, friends } = user
    ...
    useEffect(() => {
        ...
        let getPosts = async () => {
            const { data } = await axiosClient.get("/posts?type=user")
        ...
        let getFriends = async () => {
            ...
            const response = await axiosClient.get("user/followers")
            setFollowersData(response.data.followers)
            setFriendsData(data.friends.friends)
        }
        getFriends()
    }, [])
    const uploadContent = async ({ content, media, formData, multiple }) => {
        setNewPost({ content, media, username, user, time: Date.now() })
        ...
        const response = await axiosClient.post("/posts/create", { postDetails })
    ...
    const setMediaModelDetails = (media) => {
            ...
            <div className='flex w-full flex-col items-center w-ful'>
                ...
                <div className='max-w-xl w-full flex flex-col gap-2 relative '>
                            ...
                            <div className='flex w-full items-center flex-col gap-2 '>
                                {posts?.map((post) => (
                                    <Post postData={post} username={username} userId={user?._id} removePost={removePost} />
                                ))}
