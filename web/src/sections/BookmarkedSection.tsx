        ...
        <div className='w-full flex md:px-6 lg:px-24 overflow-y-auto border-muted'>
            <div className='max-w-xl w-full flex flex-col gap-2'>
                ...
                <div className='relative flex w-full items-center  flex-col gap-2 '>
                    ...
                    {!isLoading && data.length > 0 && data[0].bookmarks.length > 0 ? data.map((page, i) => {
                        return page.bookmarks.map((postData, postIndex) => {
                            console.log(postData, 'this is postdata book')
                            return (< Post useLikePost={useLikeBookmarkedPost} useBookmarkPost={useBookmarkPost} pageIndex={i} postIndex={postIndex} postData={{ ...postData?.post, target: postData?.target, user: postData?.user }} userId={user?._id} username={user?.username} profile={user?.images?.profile} key={postData?.post?._id} type={postData?.type} />)
                        })
                    })
                        :
                        <div className=' flex flex-col w-full relative top-[80%]  items-center justify-center'>
                            ...
                            <span className='text-center'>No Bookmarked Posts.</span>
                        </div>
                    }
                </div>
            </div>
