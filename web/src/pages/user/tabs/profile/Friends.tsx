...
function Friends({ userFriends, isSelf, removeFriend, media, setMediaModelDetails, setMediaOpenDetails, setMediaOpenModel }) {
    const { ref, inView } = useInView()

    useEffect(() => {
        userFriends.fetchNextPage()
    }, [inView])
            ...
            <div className='max-w-xl w-full flex flex-col gap-2 '>
                <div className='flex w-full items-center  flex-col gap-2 '>
                    {userFriends.isSuccess && userFriends.data?.map((page, pageIndex) => {
                        return page.friends.map((friend, friendIndex) => {
